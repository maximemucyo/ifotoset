...
import { usePhotographerProfile, usePhotographerReviews, useSubmitPhotographerReview } from '@/lib/queries/public'
...
function NotFound({ username }: { username: string }) {
  return (
    <div className="min-h-screen bg-background flex items-center justify-center text-center px-4 text-foreground">
      <div>
        <Camera size={56} className="text-muted-foreground/30 mx-auto mb-4" />
        <h1 className="text-2xl font-bold font-serif">Photographer not found</h1>
        <p className="text-muted-foreground mt-2">No photographer with username @{username} was found.</p>
        <Link href="/" className="mt-6 inline-block px-6 py-3 bg-primary text-primary-foreground rounded-xl font-bold transition-transform hover:scale-105">
          Back to ifotoset
        </Link>
      </div>
    </div>
...
export default function PhotographerLandingPage() {
  ...
  const { data: reviewsData } = usePhotographerReviews(username)
  ...
  const { categories, filteredGalleries, coverImageUrl, firstGallery } = useMemo(() => {
    const galleries = data?.featured_galleries ?? []
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center text-center px-4 text-foreground">
        <div>
          ...
          <p className="text-muted-foreground mt-2 text-sm">{(error as any)?.message}</p>
        </div>
      </div>
    )
  }
