...
export default function PublicGalleryView() {
        ...
        <div className="bg-card border border-border rounded-2xl p-8 max-w-md w-full shadow-lg text-center space-y-6">
          ...
          <form onSubmit={handleUnlockSubmit} className="space-y-4">
            <input
              ...
              className="w-full px-4 py-3 bg-background border border-border rounded-xl placeholder-muted-foreground/60 focus:outline-none focus:border-primary transition-colors"
            />
            {unlockError && <p className="text-xs text-destructive text-left font-medium">{unlockError}</p>}
            <button
              type="submit"
              disabled={unlocking}
              className="w-full py-3 bg-primary text-primary-foreground rounded-xl hover:bg-primary/95 font-semibold transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
            >
  ...
  return (
    <div className="min-h-screen bg-background text-foreground transition-colors duration-200">
      ...
      <header className="border-b border-border bg-card/85 backdrop-blur-md sticky top-0 z-30">
        <div className="w-full max-w-none px-4 md:px-8 xl:px-12 2xl:px-16 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-bold text-lg text-foreground truncate max-w-[200px] sm:max-w-sm">
              {gallery.title}
            </h1>
          </div>
      ...
      {/* Hero Banner Section */}
      <section className="bg-gradient-to-b from-secondary/20 via-transparent to-transparent border-b border-border py-16 px-6 text-center">
        <div className="max-w-3xl mx-auto space-y-3">
          <h2 className="text-4xl font-extrabold text-foreground tracking-tight sm:text-5xl">
            {gallery.title}
          </h2>
          <p className="text-md text-muted-foreground max-w-xl mx-auto mt-3">
      ...
      {isEmailModalOpen && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setIsEmailModalOpen(false)}>
          <div className="bg-card border border-border rounded-2xl p-6 max-w-sm w-full shadow-xl space-y-4 animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-bold text-foreground">Save Your Favorites</h3>
              <button onClick={() => setIsEmailModalOpen(false)} aria-label="Close modal" className="text-muted-foreground hover:text-foreground p-1 rounded-lg hover:bg-secondary/35 transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-3">
              <input
                ...
                className="w-full px-3 py-2.5 bg-background border border-border rounded-xl text-sm placeholder-muted-foreground/60 focus:outline-none focus:border-primary transition-colors text-foreground"
              />
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2.5 bg-secondary text-secondary-foreground hover:bg-secondary/80 rounded-xl text-xs font-semibold transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-primary text-primary-foreground hover:bg-accent rounded-xl text-xs font-semibold transition-colors"
                >
                  Save & Favorite
                </button>
              </div>
      ...
      {sharingPhoto && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSharingPhoto(null)}>
            <div className="bg-card border border-border rounded-2xl p-6 max-w-sm w-full shadow-xl space-y-5 animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-bold text-foreground">Share Photo</h3>
                <button onClick={() => setSharingPhoto(null)} aria-label="Close modal" className="text-muted-foreground hover:text-foreground p-1 rounded-lg hover:bg-secondary/35 transition-colors">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 min-w-0 px-3 py-2 bg-secondary/30 border border-border rounded-xl text-xs text-muted-foreground focus:outline-none select-all"
                />
                <button
                  onClick={() => {
                  ...
                  className="px-4 py-2 bg-primary text-primary-foreground hover:bg-accent font-semibold rounded-xl text-xs transition-colors shrink-0 flex items-center gap-1.5 min-w-[90px] justify-center"
                >
                  {isCopied ? (
                    <>
                      <Check size={14} />
              ...
              <div className="grid grid-cols-4 gap-3 py-2">
                ...
                <a
                  ...
                  className="flex flex-col items-center gap-1.5 p-2 rounded-xl hover:bg-secondary/25 transition-colors text-muted-foreground hover:text-foreground"
                >
                ...
                <a
                  ...
                  className="flex flex-col items-center gap-1.5 p-2 rounded-xl hover:bg-secondary/25 transition-colors text-muted-foreground hover:text-foreground"
                >
                  <div className="w-10 h-10 rounded-full bg-slate-900/10 dark:bg-white/10 flex items-center justify-center text-foreground">
                ...
                <a
                  ...
                  className="flex flex-col items-center gap-1.5 p-2 rounded-xl hover:bg-secondary/25 transition-colors text-muted-foreground hover:text-foreground"
                >
                  <div className="w-10 h-10 rounded-full bg-black/10 dark:bg-white/10 flex items-center justify-center text-foreground">
                    <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
